...
[![Open in Streamlit](https://static.streamlit.io/badges/streamlit_badge_black_white.svg)](https://share.streamlit.io/deploy?repository=mungn0603-code%2Fchemical_reactor_parameter_optimization_platform&branch=main&mainModule=app.py)
...
# 테스트(44)
pytest reactor_platform/tests -v
```

## 품질 & CI

모든 push·PR에서 GitHub Actions가 Ruff(린트) · mypy(타입) · pytest(3.10/3.11/3.12 매트릭스) · Coverage 배지를 실행한다.

## 로드맵

M0/M1b(CSTR) · Scenario Lab · M2(열역학) · M3(에너지수지) 완료. 향후: M4(경제성 NPV/IRR), M5(최적화), Batch/PFR, AI 추천, Digital Twin.
