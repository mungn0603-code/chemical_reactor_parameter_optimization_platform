...
- **🧪 반응기 계산 (M1→M2→M3 순차)** — 속도론/CSTR → 열역학 → 에너지수지를 순서대로 진행.
  **반응식을 선택**(예: `N2 + 3 H2 -> 2 NH3`)하면 하나의 반응이 전 단계를 구동한다.
  - **M1**: 빈도인자 A·활성화에너지 Ea·반응차수 n 이 **문헌 대표값(kinetics DB)** 으로 자동 반영
    (⚠ A·Ea 는 반응식에서 유도되지 않는 실험값이라 계산이 아닌 DB 조회. 수동 수정 가능).
  - **M2**: 화학종을 열화학 DB 에 매핑해 **Hess 법칙으로 ΔH_rxn°·ΔS_rxn°·ΔG°·K_eq 를
    교과서대로 자동 계산**(원소 균형 자동 검사).
  - **M3**: 반응열 ΔH_rxn 이 그대로 반영되어 열부하 Q·가열/냉각을 계산.
...
- **반응속도 파라미터 자동 반영** — 반응 선택 시 빈도인자 A·활성화에너지 Ea·차수를 문헌 대표값(kinetics DB)으로 자동 설정. 열역학과 달리 A·Ea 는 실험값이므로 계산이 아닌 DB 조회임을 UI 에 명시.
